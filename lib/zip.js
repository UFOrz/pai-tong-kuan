...
export async function extractZipEntries(blob, {
  ...
  if (entryCount > maxEntries) throw new Error(`ZIP 内文件过多，最多支持 ${maxEntries} 个`);
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (uncompressedSize > maxEntrySize) throw new Error('ZIP 内单个文件过大，已停止解压');
    totalSize += uncompressedSize;
    if (totalSize > maxTotalSize) throw new Error('ZIP 解压后体积过大，已停止解压');
