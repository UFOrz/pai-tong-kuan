...
async function runningHubWorkflowResultFromUrl(url, {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      const images = await imageResultsFromRunningHubWorkflowDownload(await response.blob(), url);
...
function atlasCloudNanoBananaRequestBody({ model, body, ratio, resolution, images }) {
  const match = /^google\/(nano-banana-2-lite|nano-banana-2|nano-banana-pro|nano-banana)\/(text-to-image(?:-developer)?|edit(?:-developer)?)$/i.exec(String(model || ''));
