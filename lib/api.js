...
export function surprisePromptRequestText({ genre, recentHistory = [] } = {}) {
  ...
  const recent = (Array.isArray(recentHistory) ? recentHistory : [])
    ...
    .map((item, index) => {
      ...
      const prompt = String(item?.prompt || '').replace(/\s+/g, ' ').trim().slice(0, 600);
      return `${index + 1}. [${label}] ${prompt || '(category was reserved but no output was saved)'}`;
    });
  return [
    `SELECTED CATEGORY: ${selected.label}`,
    `CATEGORY DIRECTION: ${selected.direction}`,
    '',
    'RECENT OUTPUTS TO AVOID:',
    ...(recent.length ? recent : ['None.']),
    '',
    'Generate one new prompt now. Stay inside the selected category and make the central subject, setting, action, composition, era or region, and color palette clearly different from the recent outputs.'
  ].join('\n');
}
