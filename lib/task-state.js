...
export function generationJobBelongsToSource(job, source) {
  const jobSourceKey = job?.sourceKey || null;
  const currentSourceKey = sourceKey(source);
  return Boolean(jobSourceKey && currentSourceKey && jobSourceKey === currentSourceKey);
}

export function completedGenerationJobsForSource(jobs, source) {
  return (Array.isArray(jobs) ? jobs : [])
    .filter((job) => job?.status === 'completed' && generationJobBelongsToSource(job, source))
    .sort((a, b) =>
      (Number(a.finishedAt || a.updatedAt || a.startedAt) || 0) -
