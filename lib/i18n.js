...
const JA = {
  ...
  , '只有当用户点击反推、生成、替换角色或组图生成时，扩展才会将完成任务所必需的来源图、参考素材和提示词发送到用户选定的 AI 平台。': 'ユーザーが解析、生成、人物置換、画像セット生成を開始した場合に限り、タスクに必要な元画像、参照素材、プロンプトを選択したAIサービスへ送信します。', 'API Key 仅用于向该平台鉴权。扩展开发者不运营中转服务器，也不会接收用户的 API Key、图片或提示词。': 'APIキーは選択先サービスの認証にのみ使用されます。開発者は中継サーバーを運営せず、APIキー、画像、プロンプトを受信しません。', '内置平台包括 OpenAI、ModelScope、SiliconFlow、Agnes-AI、ZenMux、RunningHUB、AtlasCloud、APImart 和 OpenRouter；用户也可以配置自己的兼容服务。': '内蔵サービスはOpenAI、ModelScope、SiliconFlow、Agnes-AI、ZenMux、RunningHUB、AtlasCloud、APImart、OpenRouterです。互換サービスも設定できます。', '第三方平台如何保存和处理请求，由相应平台的隐私政策及用户与该平台之间的协议决定。': '第三者サービスによるリクエストの保存と処理は、各サービスのプライバシーポリシーとユーザー契約に従います。', '除完成用户明确请求的反推、生图，或法律、安全所必需的情形外，扩展不会向其他第三方传输数据。': '明示されたタスクの実行、法令または安全上必要な場合を除き、他の第三者へデータを送信しません。'
...
const KO = {
  ...
  , '只有当用户点击反推、生成、替换角色或组图生成时，扩展才会将完成任务所必需的来源图、参考素材和提示词发送到用户选定的 AI 平台。': '사용자가 분석, 생성, 인물 교체 또는 이미지 세트 작업을 시작할 때만 필요한 원본 이미지, 참고 소재 및 프롬프트를 선택한 AI 플랫폼으로 전송합니다.', 'API Key 仅用于向该平台鉴权。扩展开发者不运营中转服务器，也不会接收用户的 API Key、图片或提示词。': 'API 키는 선택한 플랫폼 인증에만 사용됩니다. 개발자는 중계 서버를 운영하지 않으며 사용자의 API 키, 이미지 또는 프롬프트를 받지 않습니다.', '内置平台包括 OpenAI、ModelScope、SiliconFlow、Agnes-AI、ZenMux、RunningHUB、AtlasCloud、APImart 和 OpenRouter；用户也可以配置自己的兼容服务。': '내장 플랫폼에는 OpenAI, ModelScope, SiliconFlow, Agnes-AI, ZenMux, RunningHUB, AtlasCloud, APImart 및 OpenRouter가 있으며 호환 서비스를 직접 설정할 수도 있습니다.', '第三方平台如何保存和处理请求，由相应平台的隐私政策及用户与该平台之间的协议决定。': '타사 플랫폼의 요청 저장 및 처리는 각 플랫폼의 개인정보 처리방침과 사용자 계약을 따릅니다.', '除完成用户明确请求的反推、生图，或法律、安全所必需的情形外，扩展不会向其他第三方传输数据。': '사용자가 명시적으로 요청한 작업 수행 또는 법률·보안상 필요한 경우를 제외하고 다른 제3자에게 데이터를 전송하지 않습니다.'
...
Object.assign(EN, {
  ...
  '不是拍同款配置文件': 'This is not a Match This settings file',
...
Object.assign(JA, {
  ...
  '不是拍同款配置文件': '「同じ画像を作る」の設定ファイルではありません',
...
Object.assign(KO, {
  ...
  '不是拍同款配置文件': '같은 이미지 만들기 설정 파일이 아닙니다',
